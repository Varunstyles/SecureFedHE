...
Main entry point for Ring 2. Run AFTER Ring 1 baseline is confirmed.

Usage:
    python -m baseline.he_train
    python -m baseline.he_train --rounds 20 --clients 5 --epsilon 10
    python -m baseline.he_train --rounds 20 --clients 5 --epsilon 50

What this produces:
    evaluation/logs/he_eps{X}_metrics.csv  ← same schema as baseline_metrics.csv
    evaluation/logs/best_he_eps{X}.pth     ← best model weights
"""
...
def parse_args():
    ...
    p.add_argument("--validate-he",action="store_true",
                   help="Print HE approximation error each round")
    p.add_argument("--data-dir",   type=str,   default="./data/cifar10")
    p.add_argument("--log",        type=str,   default=None,
                   help="Log path (auto-generated if not set)")
...
def main():
    ...
    # Auto-generate log path based on epsilon
    if args.log is None:
        eps_tag = str(int(args.epsilon)) if args.epsilon == int(args.epsilon) else str(args.epsilon)
        args.log = f"evaluation/logs/he_eps{eps_tag}_metrics.csv"
    ...
    print(f"  DP eps={args.epsilon}, delta={args.delta} | HE degree={args.he_degree}")
    ...
    for rnd in range(1, args.rounds + 1):
        ...
        if eval_acc > best_acc:
            ...
            torch.save(global_model.state_dict(),
                       f"evaluation/logs/best_he_eps{eps_tag}.pth")
    ...
    # -- Summary -----------------------------------------------------------
    print(f"\n{'='*65}")
    print(f"  Ring 2 complete (eps={args.epsilon}).")
    print(f"  Best test accuracy    : {best_acc*100:.2f}%")
    print(f"  Metrics log           : {args.log}")
    print(f"{'='*65}")
    print(f"\n  [OK]  Ring 2 (eps={args.epsilon}) validated.\n")
